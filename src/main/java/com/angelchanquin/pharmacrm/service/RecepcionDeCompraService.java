...
public class RecepcionDeCompraService {
    ...
    public List<RecepcionDeCompra> findAll() {
        log.debug("Request to get all RecepcionDeCompras");
        return recepcionDeCompraRepository.findAll();
    }
     ...
     */
    @Transactional(readOnly = true)
    public List<RecepcionDeCompra> search(String query) {
        log.debug("Request to search RecepcionDeCompras for query {}", query);
        return StreamSupport
            .stream(recepcionDeCompraSearchRepository.search(queryStringQuery(query)).spliterator(), false)
